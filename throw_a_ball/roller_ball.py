"""Roller Ball rules and target geometry for the first Throw a Ball prototype."""
...
def resolve_arcade_shot(x: int, y: int) -> ShotResult:
    """Map one raw 128x128 dart hit to the Roller Ball placeholder playfield."""
    ...
    if pocket_hits:
        ...
        return ShotResult(x, y, pocket.x, pocket.y, pocket.score, pocket.name)

    cx, cy = TEN_CATCH_CENTER
    normalized = ((x - cx) / TEN_CATCH_RADIUS_X) ** 2 + ((y - cy) / TEN_CATCH_RADIUS_Y) ** 2
    if normalized <= 1.0 and y >= 47:
        return ShotResult(x, y, 64, 101, 10, "10")

    return ShotResult(x, y, x, min(y, 112), 0, "MISS")


def sample_ball_position(shot: ShotResult, progress: float) -> tuple[int, int]:
    """Return a simple arcade roll trajectory from the lane foot to the target."""
